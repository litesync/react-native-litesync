...
SQLitePluginTransaction = function(db, fn, error, success, txlock, readOnly) {
  ...
  if (txlock) {
    ...
    // we should be able to run a PRAGMA command before any other
...
check_if_ready = function(db, callback) {
  db.executeSql("pragma sync_status", [], (results) => {
    if (results.rows && results.rows.length > 0) {
      var status = results.rows.item(0);
      status = JSON.parse(status.sync_status);
      callback(status.db_is_ready);
    } else {
      fire_error_cb(db, 'LiteSync is not active')
    }
  }, (msg) => {
    fire_error_cb(db, 'could not run "pragma sync_status": ' + msg)
  });
}

fire_error_cb = function(db, msg) {
  if (!db.events) return;
  var callback = db.events['on_error']
  if (callback) {
    callback(msg)
  }
}

SQLitePlugin.prototype.on = function(name,callback) {

  var db = this

  if (name=='not_ready') {
    check_if_ready(db, (is_ready) => {
      if (is_ready===false) callback();
    })
    return;
  }

  if (!(name=='ready' || name=='sync' || name=='error')) {
  ...
  if (!db.udp_socket) {
    db.udp_socket = dgram.createSocket('udp4');

    db.udp_socket.on('message',function(msg,info){
      var event = String.fromCharCode.apply(null, new Uint8Array(msg));
    ...
    db.udp_socket.once('listening',function(){
      let address = db.udp_socket.address();
      let port = address.port;
      db.executeSql('PRAGMA enable_notifications="udp:' + port + '"', [])
    });
    ...
    function randomPort() {
      var minp = 1025
      var maxp = 65530
      return parseInt(Math.random() * (maxp - minp)) + minp
    }

    function bind_to_random_port(){
      var port = randomPort();
      db.udp_socket.bind(port, (err) => {
        if (err) {
          bind_to_random_port()
        }
      });
